...
// API
import quantfactorsApi from '@/pages/Store/api/quantfactors'
...
const ToolDialog = ({ show, dialogProps, onUseTemplate, onCancel, onConfirm, setError }) => {
    ...
    const getSpecificQuantfactorApi = useApi(quantfactorsApi.getSpecificQuantfactor)
    ...
    useEffect(() => {
        if (getSpecificQuantfactorApi.data) {
            setToolId(getSpecificQuantfactorApi.data.id)
            setToolName(getSpecificQuantfactorApi.data.name)
            setToolDesc(getSpecificQuantfactorApi.data.description)
            setToolSchema(formatDataGridRows(getSpecificQuantfactorApi.data.schema))
            if (getSpecificQuantfactorApi.data.func) setToolFunc(getSpecificQuantfactorApi.data.func)
            else setToolFunc('')
        }
    }, [getSpecificQuantfactorApi.data])

    useEffect(() => {
        if (getSpecificQuantfactorApi.error) {
            setError(getSpecificQuantfactorApi.error)
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [getSpecificQuantfactorApi.error])
    ...
    useEffect(() => {
        if (dialogProps.type === 'EDIT' && dialogProps.data) {
            ...
            getSpecificQuantfactorApi.request(dialogProps.toolId)
    ...
    const exportTool = async () => {
        try {
            const toolResp = await quantfactorsApi.getSpecificQuantfactor(toolId)
            if (toolResp.data) {
    ...
    const addNewTool = async () => {
        try {
            ...
            const createResp = await quantfactorsApi.createNewTool(obj)
            if (createResp.data) {
                enqueueSnackbar({
    ...
    const saveTool = async () => {
        try {
            const saveResp = await quantfactorsApi.updateTool(toolId, {
                name: toolName,
                description: toolDesc,
                schema: JSON.stringify(toolSchema),
                func: toolFunc,
