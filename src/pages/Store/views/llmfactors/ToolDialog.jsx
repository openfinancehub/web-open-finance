...
// API
import llmfactorsApi from '@/pages/Store/api/tools'
...
const ToolDialog = ({ show, dialogProps, onUseTemplate, onCancel, onConfirm, setError }) => {
    ...
    const getSpecificLlmfactorApi = useApi(llmfactorsApi.getSpecificLlmfactor)
    ...
    useEffect(() => {
        if (getSpecificLlmfactorApi.data) {
            setToolId(getSpecificLlmfactorApi.data.id)
            setToolName(getSpecificLlmfactorApi.data.name)
            setToolDesc(getSpecificLlmfactorApi.data.description)
            setToolSchema(formatDataGridRows(getSpecificLlmfactorApi.data.schema))
            if (getSpecificLlmfactorApi.data.func) setToolFunc(getSpecificLlmfactorApi.data.func)
            else setToolFunc('')
        }
    }, [getSpecificLlmfactorApi.data])

    useEffect(() => {
        if (getSpecificLlmfactorApi.error) {
            setError(getSpecificLlmfactorApi.error)
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [getSpecificLlmfactorApi.error])
    ...
    useEffect(() => {
        if (dialogProps.type === 'EDIT' && dialogProps.data) {
            ...
            getSpecificLlmfactorApi.request(dialogProps.toolId)
    ...
    const exportTool = async () => {
        try {
            const toolResp = await llmfactorsApi.getSpecificLlmfactor(toolId)
            if (toolResp.data) {
    ...
    const addNewTool = async () => {
        try {
            ...
            const createResp = await llmfactorsApi.createNewTool(obj)
            if (createResp.data) {
                enqueueSnackbar({
    ...
    const saveTool = async () => {
        try {
            const saveResp = await llmfactorsApi.updateTool(toolId, {
                name: toolName,
                description: toolDesc,
                schema: JSON.stringify(toolSchema),
                func: toolFunc,
    ...
    const deleteTool = async () => {
        ...
        if (isConfirmed) {
            try {
                const delResp = await llmfactorsApi.deleteTool(toolId)
                if (delResp.data) {
                    enqueueSnackbar({
