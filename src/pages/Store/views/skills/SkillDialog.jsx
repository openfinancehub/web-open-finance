...
const SkillDialog = ({ show, dialogProps, onUseTemplate, onCancel, onConfirm, setError }) => {
    ...
    const getSpecificToolApi = useApi(skillsApi.getSpecificTool)
    ...
    useEffect(() => {
        if (dialogProps.type === 'EDIT' && dialogProps.data) {
            // When tool dialog is opened from Skills dashboard
    ...
    const exportTool = async () => {
        try {
            const toolResp = await skillsApi.getSpecificTool(toolId)
            if (toolResp.data) {
    ...
    const addNewTool = async () => {
        try {
            ...
            const createResp = await skillsApi.createNewTool(obj)
            if (createResp.data) {
                enqueueSnackbar({
    ...
    const saveTool = async () => {
        try {
            const saveResp = await skillsApi.updateTool(toolId, {
                name: toolName,
                description: toolDesc,
                schema: JSON.stringify(toolSchema),
                func: toolFunc,
    ...
    const deleteTool = async () => {
        ...
        if (isConfirmed) {
            try {
                const delResp = await skillsApi.deleteTool(toolId)
                if (delResp.data) {
                    enqueueSnackbar({
