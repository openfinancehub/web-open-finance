...
const Factor: React.FC<MyComponentProps> = () => {
  ...
  const handleButtonChange = (
    ...
    butttonId: React.SetStateAction<string>,
    buttonNum: any
  ) => {
            ...
            return (
              <div key={item.name}>
                <Space>
                  ...
                  <Select
                    onChange={value => {
                      handleProvinceChange(item.measures, value);
                    }}
                    defaultValue={item.measures[0].desc}
                    style={{ width: 120 }}>
                    {item.measures.map(province => {
                      return (
                        <Option key={province.desc} value={province.desc}>
                          {province.desc}
                        </Option>
                      );
                    })}
                  ...
                  <Link to={`/analyze/factordelite?id=${14}`}>
                    <Button type="primary" size={size}>
                      详情
                    </Button>
                  </Link>
                  <Popover
                    content={<div style={{ width: '500px' }}>{item.desc}</div>}
                    title="描述">
                    <Button
                      size={size}
                      type="primary"
